    ...
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProxyProvider<AuthProvider, OrdersProvider>(
          ...
          update: (ctx, authProvider, ordersProvider) => OrdersProvider(orders: ordersProvider?.orders, token: authProvider.token, userId: authProvider.userId),
        ),
      ...
      child: Consumer<AuthProvider>(
        builder: (ctx, authProvider, _) => MaterialApp(
          ...
          routes: {
            rootRouteName: (ctx) => authProvider.isAuth
                ? const ProductsOverviewScreen()
                : FutureBuilder(
                    future: authProvider.tryAutologin(),
                    builder: (ctx, authProvider) {
                      return authProvider.connectionState == ConnectionState.waiting ? const SplashScreen() : const AuthScreen();
                    },
                  ),
